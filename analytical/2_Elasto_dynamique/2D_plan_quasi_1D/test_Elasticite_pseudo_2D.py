...
damping_dic = {"damping" : True, "linear_coeff" : 0.1, "quad_coeff" : 0.1, "correction" : True}
...
temps = loadtxt(output_name+"-results/export_times.csv",  delimiter=',', skiprows=1)[1:]
resultat = [df[colonne].to_numpy() for colonne in df.columns]
sigma_xx = [resultat[3 * i + 2] for i in range((len(resultat)-2)//3)]
pas_espace = linspace(0, Longueur, len(sigma_xx[0]))
